...
echo $this->Html->css('cancelar.css');
...
<body style="text-align: center; margin-left: 16%;">
    ...
    <h2 class="page-header">Cancelar Venda</h2>
    <input class="form-control" id="myInput" type="text" placeholder="Busca" style="width: 20%; height: 30px; margin-left: 40%;">
    <br />
    <table class="table table-striped" cellpadding="0" cellspacing="0">
        <thead>
            <tr>
                <th scope="col" class="th-cancelar"><?= $this->Paginator->sort('ID') ?></th>
                <th scope="col" class="th-cancelar"><?= $this->Paginator->sort('Cliente') ?></th>
                <th scope="col" class="th-cancelar"><?= $this->Paginator->sort('Total de produto') ?></th>
                <th scope="col" class="th-cancelar"><?= $this->Paginator->sort('Total da Venda') ?></th>
                <th scope="col" class="th-cancelar"><?= $this->Paginator->sort('Quant. de Pagamentos') ?></th>
                <th scope="col" class="th-cancelar"><?= $this->Paginator->sort('Data da venda') ?></th>
                <th scope="col" class="actions th-cancelar"><?= __('') ?></th>
            </tr>
        </thead>
        <tbody  id="myTable">
            <?php  
            ...
            if(isset($results[0])){
                ...
                if($role == "caixa"){
                    ...
                    $results = $query->toArray();
                    if(isset($results[0])){
                        $idCaixa = $results[0]["id"];
                        if($venda->caixas_id == $idCaixa){  
                            if(($venda->venda_cancelada) == 0) { ?>
                                <form action="/projeto/vendas/cancelar/" method="post">
                                <?= $this->Form->create() ?>
                                <tr>
                                    <td><?= $this->Number->format($venda->id) ?></td>
                                    <td><?= h($venda->cliente->name) ?></td>
                                    <td><?= $this->Number->format($venda->total_produto) ?></td>
                                    <td><?= h($venda->total_venda) ?></td>
                                    <td><?= h($venda->total_pagamentos) ?></td>
                                    <td><?= date("d/m/Y H:m:s", strtotime($venda->created)); ?></td>
                                    <td>
                                        <input type="hidden" id="id" name="venda" class="checkTodos" value="<?php echo $venda['id'];?>">
                                        <!-- Button to Open the Modal -->
                                        <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#<?php echo $venda['id'];?>">
                                            Cancelar
                                        </button>

                                        <!-- The Modal -->
                                        <div class="modal" id="<?php echo $venda['id'];?>"> 
                                            <!-- id da modal pra cada form-->
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-body">
                                                        ...
                                                        <textarea class="form-control" rows="3" name="observacao_venda_cancelada" value=""></textarea>
                                                        
                                                        <?php
                                                        date_default_timezone_set('America/Sao_Paulo');
                                                        $data_cancelamento = date("Y-m-d H:i:s");
                                                        echo '<input type="hidden" name="data_cancelamento" value="'.$data_cancelamento.'">';
                                                        ?>
                                                    </div>

                                                    <!-- Modal footer -->
                                                    <div class="modal-footer">
                                                        <button type="submit" class="btn-success btn cancelar">Cancelar</button>
