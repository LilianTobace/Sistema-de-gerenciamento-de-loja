...
<body class="relatorio-venda">
    <h2 class="page-header">Gráfico de Ticket Médio</h2>
    <br />
    <?php 
    if(isset($_SESSION['listaFiltrada'])){ 
        $totalCliente = count($_SESSION['listaFiltrada']);
        $totalGasto = $_SESSION['totalGasto'];
        $media = ($totalGasto/$totalCliente);
        ?>
       <p class="p3">Total de Clientes: <?php echo $totalCliente ?></p>
       <p class="p2">Gastos total: R$<?php echo round($_SESSION['totalGasto'], 2) ?></p>
       <p class="p2">Média de Gasto:  R$<?php echo round($media,2) ?>
            <div class="div1">
