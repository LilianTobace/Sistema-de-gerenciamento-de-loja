...
<body class="relatorio-venda">
    <h2 class="page-header">Relatório de Pagamento</h2>
    <?= $this->Form->create($vendasPagas) ?>
        ...
        <table class="table table-striped" cellpadding="0" cellspacing="0">
            <thead>
                <tr>
                    ...
                    <th class="th-venda"><?= $this->Paginator->sort('Venda ID'); ?></th>
                    <th class="th-venda"><?= $this->Paginator->sort('Forma de Pagamento'); ?></th>
                    <th class="th-venda"><?= $this->Paginator->sort('Parcelas'); ?></th>
                    <th class="th-venda"><?= $this->Paginator->sort('Valor Pago'); ?></th>
                    <th class="th-venda"><?= $this->Paginator->sort('Data do Pagamento'); ?></th>
                </tr>
            </thead>
            ...
            <tbody>
                <?php if(isset($_SESSION['listaFiltrada'])){ 
                    ...
                    <tr>
                        ...
                        ?>
                        <td><?php echo $formaPag ?></td>
                        <td><?php echo $listaFiltrada[$i]['parcelas'] ?>x</td>
                        <td>R$<?php echo $listaFiltrada[$i]['valor_pago'] ?></td>
                ...
                <?php
                    ...
                    foreach ($vendasPagas as $vendasPaga):
                    unset($_SESSION['listaFiltrada']);
                ?>
                    <tr>
                        <td><?= $this->Number->format($vendasPaga->id) ?></td>
                        <td><?= $this->Number->format($vendasPaga->vendas_id) ?></td>
                        <?php         
                        $pagamentos = TableRegistry::getTableLocator()->get('Pagamentos');
                        $query = $pagamentos
                            ->find()
                            ->where([
                                    'id' => $vendasPaga->pagamentos_id
                            ]);
                        $results = $query->toArray();
                        if(isset($results[0])){                
                            $formaPag = $results[0]["name"];
                        ...
                        ?>
                        <td><?php echo $formaPag ?></td>
                        <td><?= h($vendasPaga->parcelas) ?>x</td>
                        <td>R$<?= h($vendasPaga->valor_pago) ?></td>
                        <?php  $criado =  date("d/m/Y H:m:s", strtotime($vendasPaga->created));?>
                        <td><?php echo $criado; ?></td>
                    </tr>
                <?php 
                endforeach;
    ...
    <table id="resultadoFiltro" class="table resultado_relatorio">
        <tr>
            ...
            ?>
            <th class="resultado_relatorio1">
                Forma de Pagamento mais realizada: <?php echo $_SESSION['nome']; ?>
            </th>
            <?php
            $vendasPagas = TableRegistry::getTableLocator()->get('VendasPagas');
            $query = $vendasPagas->find()
            ...
            ?>
            <th class="resultado_relatorio1">
                Total Pago em <?php echo $_SESSION['nome']; ?>: R$<?php echo $_SESSION['valorPago']; ?>
            </th>
        </tr>
    </table> 
</body>
