...
<body class="relatorio-venda">
    <h2 class="page-header">Gráfico de Cliente</h2>
    <br />
    <?php if(isset($_SESSION['listaFiltrada'])){ ?>
       <p class="p1">Quantidade de produto: <?php echo $_SESSION['produtoTotal'] ?></p>
       <p class="p2">Valor Total:  R$<?php echo $_SESSION['totalVenda'] ?></p>
            <div class="div1">
            ...
            var myChart = new Chart(ctx, {
                ...
                options:{
                    ...
                    for ($i=0; $i<count($listaFiltrada); $i++) {
                        $nome = $_SESSION['nomeCliente'][$i];
                        $created = $listaFiltrada[$i]['created'];?>",
                    "<?php 
                    echo $nome;
                    }
                    ?>"],
